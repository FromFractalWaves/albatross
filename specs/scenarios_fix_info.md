...
- Displays tiers as sections with formatted headers (e.g. "TIER ONE") and scenario counts
- Each scenario is a clickable row linking to `/trm/scenarios/{tier}/{scenario}`
- Has three tabs: SCENARIOS (active), LIVE (disabled), HISTORY (disabled)
...
1. **Homepage** (`/`) → Click "Browse Scenarios" on TRM Tools card
