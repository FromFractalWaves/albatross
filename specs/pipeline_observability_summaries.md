...
A new `PipelineStageDefinition` type in `contracts/ws.py` defines each stage with `id`, `label`, and `message_type`. `PipelineStarted` is changed: `total_packets` is removed and a `stages` field is added carrying a list of stage definitions. `LivePipelineManager` declares its three stages (capture, preprocessing, routing) via the `pipeline_stages` property and includes them in the `pipeline_started` broadcast.
...
On the frontend, `PipelineStartedMessage` in `websocket.ts` drops `total_packets` and gains a `stages` field. A new `PipelineStageDefinition` type is added. `useLiveData` maintains stage state in a dedicated TanStack Query cache key `["live", "stages"]`. On `pipeline_started`, it resets counts to 0 and updates the stages definition from the message. On `packet_captured`, `packet_preprocessed`, and `packet_routed`, it finds the matching stage by `message_type` and increments its count via `queryClient.setQueryData`. The hook returns a new `stages` array with `{ id, label, message_type, count }` per stage — no `total` field. Counts increment indefinitely from zero.

The live page seeds the `["live", "stages"]` cache from its existing status poll response (which now includes `stages`). A new `PipelineStages` component renders a compact vertical strip of stage rows between the top bar and pipeline controls. Each row shows the stage ID in a fixed color (blue, amber, purple by index) and an incrementing count. No progress bars, no totals, no done/active/idle state — the diagnostic signal is the relative lag between stage counts. The component receives only `stages` (no `pipelineStatus` prop) and returns null when stages is empty. Background is `#060610` with a border-bottom matching the top bar.
